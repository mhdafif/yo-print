...
export const fetchAnimeList = createAsyncThunk(
  ...
  async ({ query, page, sortBy, signal }: { query: string; page: number; sortBy: 'rating' | 'members' | 'default'; signal?: AbortSignal }, { rejectWithValue }) => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (status === 429) {
          return rejectWithValue('The anime API is currently busy. Please try again in a moment.')
        }
        if (status && status >= 500) {
          return rejectWithValue('The anime API is experiencing issues. Please try again later.')
        }
        if (status === 404) {
          return rejectWithValue('No anime found matching your search.')
        }
        return rejectWithValue('Failed to fetch anime data. Please check your connection.')
      }
...
export const animeSearchSlice = createSlice({
  ...
  reducers: {
    ...
    setSortBy: (state, action: PayloadAction<'rating' | 'members' | 'default'>) => {
    ...
    addToSearchHistory: (state, action: PayloadAction<string>) => {
      ...
      if (query && query.length >= 3) {
        ...
        try {
          localStorage.setItem('anime-search-history', JSON.stringify(state.searchHistory))
        } catch {
...
export const { setQuery, setPage, setSortBy, addToSearchHistory, clearSearchHistory, clearError } = animeSearchSlice.actions